...
func _ensure_node(state: BoardState, id: int) -> void:
	...
	if piece == null or not is_instance_valid(piece):
		...
		# Forward checker click events if supported by the scene/script
	if piece != null and piece.has_signal("clicked"):
		var cb := Callable(self, "_on_checker_node_clicked")
		if not piece.is_connected("clicked", cb):
			piece.connect("clicked", cb)
	...
	_apply_supply_line_visual(state, id, piece)
	_apply_convoy_visual(state, id, piece)
...
func _apply_supply_line_visual(state: BoardState, id: int, piece: Node2D) -> void:
	if not piece.has_method("set_supply_line_state"):
		return
	var info: CheckerInfo = state.checkers.get(id, null)
	var active: bool = info != null and bool(info.tags.get("supply_line", false))
	piece.call("set_supply_line_state", active)

func _apply_convoy_visual(state: BoardState, id: int, piece: Node2D) -> void:
	if not piece.has_method("set_convoy_state"):
		return
	var info: CheckerInfo = state.checkers.get(id, null)
	var active: bool = info != null and bool(info.tags.get("convoy", false))
	piece.call("set_convoy_state", active)
...
func _on_checker_node_clicked(checker_id: int) -> void:
	emit_signal("checker_clicked", checker_id)
