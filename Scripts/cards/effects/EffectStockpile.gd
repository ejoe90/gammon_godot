...
func apply(round: RoundController, card: CardInstance, ctx: PatternContext) -> void:
	...
	if req == null:
		push_warning("[EffectStockpile] No ACROSS_PAIR PatternReq on card.")
		return

	var points := _find_across_pair_points(req, ctx)
	if points.size() != 2:
		push_warning("[EffectStockpile] Pattern ready but could not locate matched points.")
		return

	if round.has_method("activate_stockpile"):
		round.call("activate_stockpile", points, card)
	else:
