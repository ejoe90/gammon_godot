...
func apply(round: RoundController, card: CardInstance, ctx: PatternContext) -> void:
	...
	if req == null:
		push_warning("[EffectTunnel] No ACROSS_PAIR PatternReq on card.")
		return

	var points := _find_across_pair_points(req, ctx)
	if points.size() != 2:
		push_warning("[EffectTunnel] Pattern ready but could not locate matched points.")
		return

	if round.has_method("activate_tunnel"):
		round.call("activate_tunnel", points, heal_amount, card)
